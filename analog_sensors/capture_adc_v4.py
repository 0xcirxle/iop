...
Fastest path: RDATAC (read data continuous) mode.

We now know the chip is alive and configured correctly (proven by v2/v3).
The earlier RDATAC failure in v1 was caused by STATUS=0x06 (ACAL=1) which
puts the chip in a state where RDATAC misbehaves. v2/v3 use STATUS=0x04
(ACAL=0, BUFEN=1) and that works. v4 uses the same STATUS but with RDATAC
for max throughput.

In RDATAC mode:
  - No MUX write per sample (locked to one channel from init)
  - No SYNC/WAKEUP per sample (chip auto-converts continuously)
  - Just: wait DRDY -> clock 3 bytes out
  - CS stays LOW for the entire stream

Expect 10-15 kHz on a single channel.

Run:
    set -a; source .env; set +a
    sudo -E env "PATH=$PATH" python3 capture_adc_v4.py
"""
...
def load_config() -> Config:
    cfg = Config(
        ...
        target_rate_hz    = _i("TARGET_SAMPLE_RATE_HZ", 10_000),
    ...
    if cfg.channel not in range(8):
        raise ValueError(f"ADC_CHANNEL must be 0..7")
...
class ADS1256:
    ...
    def config_adc(self):
        """Burst-write STATUS (0x04, no ACAL), MUX, ADCON, DRATE."""
        self._wait_drdy()
        ...
        buf = [0, 0, 0, 0]
        buf[0] = (0 << 3) | (1 << 2) | (0 << 1)   # STATUS = 0x04 (BUFEN=1, ACAL=0)
        buf[1] = mux_val
    ...
    def start_continuous(self):
        """Enter RDATAC mode. CS stays LOW until stop_continuous."""
        # Wait for one DRDY first to ensure chip is ready
        self._wait_drdy()
        self._cs_low()
        self.spi.writebytes([CMD_RDATAC])
        # Datasheet t6: wait >= 50 * tCLKIN ~ 6.5 us before first read
        time.sleep(50e-6)

    def stop_continuous(self):
        try:
            # Send SDATAC to exit the continuous mode
    ...
    def read_one_continuous(self):
        """In RDATAC mode: just read 3 bytes. Caller must wait for DRDY first."""
        # readbytes is faster than xfer here since we're not transmitting
...
def run(cfg: Config):
    print("=" * 60)
    print("IOP ADC capture v4 (RDATAC continuous mode)")
    print("=" * 60)
    ...
    print("  RDATAC mode started")
    print("=" * 60)

    monotonic   = time.monotonic
    drdy_input  = GPIO.input
    drdy_pin    = DRDY_PIN
    spi_readbytes = adc.spi.readbytes
    ...
    try:
        while not _stop and (target_n is None or n < target_n):
            # Spin-wait for DRDY low
            t_wait_start = monotonic()
            while drdy_input(drdy_pin) != 0:
                if monotonic() - t_wait_start > 0.01:
                    drops += 1
                    break
            else:
                # We exited the while because DRDY went low - read sample
                buf = spi_readbytes(3)
                raw = ((buf[0] << 16) & 0xFF0000) | ((buf[1] << 8) & 0xFF00) | (buf[2] & 0xFF)
                if raw & 0x800000:
                    raw -= 1 << 24
                ...
                writerow((n, f"{t_rel:.6f}", raw, f"{volts:.6f}",
                          f"{current:.6f}"))
                n += 1
