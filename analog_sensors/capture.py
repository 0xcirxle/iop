...
class ADS1256:
    ...
    def read_one_on_channel(self, channel):
        ...
        Optimized: WREG_MUX + SYNC + WAKEUP are sent in a single SPI
        transaction (CS held low throughout). RDATA + 3-byte read is also
        a single xfer2. Net SPI syscalls per sample = 3 (was 5).

        DRDY MUST still be polled between WAKEUP and RDATA, otherwise the
        read returns the previous channel's conversion. Do not "optimize"
        that wait away.
        """
        mux_val = ((channel & 0x07) << 4) | 0x08

        # --- Club 1: WREG MUX + SYNC + WAKEUP in one CS-low window ---
        # Datasheet sec. "Command Definitions": commands are 1-byte and
        # may be issued back-to-back; WREG carries its own (cmd, count,
        # data) framing. Holding CS low across them is explicitly allowed.
        self._cs_low()
        self.spi.writebytes([
        ...
        # --- Club 2: RDATA + 3 data bytes in one xfer2 ---
        # The first byte clocks out the cmd, the next three return the
        # 24-bit conversion result (MSB first). xfer2 keeps CS asserted
        # for the whole 4-byte exchange.
        self._cs_low()
        rx = self.spi.xfer2([CMD_RDATA, 0x00, 0x00, 0x00])
        self._cs_high()

        raw = (rx[1] << 16) | (rx[2] << 8) | rx[3]
        if raw & 0x800000:
            raw -= 1 << 24
        return raw
