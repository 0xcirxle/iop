...
class ThresholdClassifier:
    ...
    Predict fault only when both imbalance.mean and neg_seq_proxy.mean cross
    their learned healthy-class thresholds.
    """
    ...
    def fit(self, X, y, feature_names):
        ...
        if int(np.sum(healthy_mask)) < 10:
            raise ValueError(
                f"Need at least 10 healthy feature vectors to fit; got {int(np.sum(healthy_mask))}"
            )
